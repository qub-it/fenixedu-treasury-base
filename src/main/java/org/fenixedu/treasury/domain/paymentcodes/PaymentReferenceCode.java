...
public class PaymentReferenceCode extends PaymentReferenceCode_Base {
	...
	@Atomic
	public static PaymentReferenceCode create(final String referenceCode, final LocalDate beginDate,
			final LocalDate endDate, final PaymentReferenceCodeStateType state, PaymentCodePool pool,
	...
	public static PaymentReferenceCode createForSibsOnlinePaymentGateway(final String referenceCode,
			final LocalDate beginDate, final LocalDate endDate, final PaymentReferenceCodeStateType state,
			PaymentCodePool pool, BigDecimal amount, final String sibsMerchantTransactionId,
			final String sibsReferenceId) {
		PaymentReferenceCode paymentReferenceCode = new PaymentReferenceCode();
		paymentReferenceCode.init(referenceCode, beginDate, endDate, state, pool, amount, amount,
	...
	private static Stream<PaymentReferenceCode> find(final FinantialInstitution finantialInstitution) {
		return finantialInstitution.getPaymentCodePoolsSet().stream()
				.flatMap(i -> i.getPaymentReferenceCodesSet().stream());
	}
	...
	public static Stream<PaymentReferenceCode> findByEndDate(final LocalDate endDate,
			FinantialInstitution finantialInstitution) {
	...
	public static Optional<PaymentReferenceCode> findUniqueBySibsMerchantTransactionId(
			final String sibsMerchantTransactionId) {
...
//        }
	...
	@Atomic
	public Set<SettlementNote> processPayment(final String responsibleUsername, BigDecimal amountToPay,
			DateTime paymentDate, String sibsTransactionId, String comments, final DateTime whenProcessedBySibs,
			final SibsReportFile sibsReportFile, final boolean checkSibsTransactionIdDuplication) {
		...
		final Set<SettlementNote> noteSet = this.getTargetPayment().processPayment(responsibleUsername, amountToPay,
				paymentDate, sibsTransactionId, comments);
		...
		for (SettlementNote settlementNote : noteSet) {
			...
			SibsTransactionDetail transactionDetail = SibsTransactionDetail.create(sibsReportFile, comments,
					whenProcessedBySibs, paymentDate, amountToPay, getPaymentCodePool().getEntityReferenceCode(),
					getReferenceCode(), sibsTransactionId, debtAccountId, customerId, businessIdentification,
	...
	public void processPaymentReferenceCodeTransaction(final SibsOnlinePaymentsGatewayLog log, PaymentStateBean bean) {
		...
		FenixFramework.atomic(() -> {
			final SibsOnlinePaymentsGateway sibsOnlinePaymentsGateway = getPaymentCodePool()
					.getSibsOnlinePaymentsGateway();
		...
		if (amount == null || !TreasuryConstants.isPositive(amount)) {
			throw new TreasuryDomainException(
					"error.PaymentReferenceCode.processPaymentReferenceCodeTransaction.invalid.amount");
		}
		...
		if (SibsTransactionDetail.isReferenceProcessingDuplicate(getReferenceCode(),
			...
			final String loggedUsername = TreasuryPlataformDependentServicesFactory.implementation()
					.getLoggedUsername();

			final Set<SettlementNote> settlementNotes = processPayment(
					StringUtils.isNotEmpty(loggedUsername) ? loggedUsername : "unknown", amount, paymentDate,
	...
	static public PaymentReferenceCode readByCode(final String code, FinantialInstitution finantialInstitution) {
		...
		for (PaymentCodePool pool : finantialInstitution.getPaymentCodePoolsSet()) {
			paymentReferenceCode = pool.getPaymentReferenceCodesSet().stream()
					.filter(y -> y.getReferenceCode().equals(code)).findFirst().orElse(null);
	...
	public SibsReportFile getReportOnDate(DateTime transactionWhenRegistered) {
		return this.getReportedInFilesSet().stream()
				.filter(x -> x.getWhenProcessedBySibs().equals(transactionWhenRegistered)).findFirst().orElse(null);
	}
	...
	public void createPaymentTargetTo(FinantialDocument finantialDocument) {
		...
		FinantialDocumentPaymentCode targetToFinantialDocument = FinantialDocumentPaymentCode.create(finantialDocument,
				this, true);
	...
	public void createPaymentTargetTo(final Set<DebitEntry> debitNoteEntries, final Set<Installment> installments,
		...
		MultipleEntriesPaymentCode target = MultipleEntriesPaymentCode.create(debitNoteEntries, installments, this,
				true);
	...
	public static PaymentReferenceCode createPaymentReferenceCodeForMultipleDebitEntries(final DebtAccount debtAccount,
		...
		for (DebitEntry entry : bean.getSelectedDebitEntries()) {
			amount = amount.add(bean.isUsePaymentAmountWithInterests() ? entry.getOpenAmountWithInterests()
					: entry.getOpenAmount());
		}
		...
		final PaymentReferenceCode paymentReferenceCode = bean.getPaymentCodePool().getReferenceCodeGenerator()
				.createPaymentReferenceCode(debtAccount, bean);
