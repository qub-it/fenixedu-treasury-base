...
public class PaymentPlansWithPenaltyTaxTest {
    ...
    private static void createPenaltyTaxAndConfigurators() {
        ...
        PaymentPlanTestsUtilities.createBlockingConfigurator(CONFIGURATOR_INT_DIST_TAX_BEFORE, Boolean.TRUE,
                AddictionsCalculeTypeEnum.BY_INSTALLMENT_ENTRY_AMOUNT, AddictionsCalculeTypeEnum.BEFORE_DEBIT_ENTRY, Boolean.TRUE);
    ...
    private static void createPenaltyTaxSettings() {
        if(!Product.findUniqueByCode("TX_PEN_ATRASO_PAG").isPresent()) {
            Product.create(ProductGroup.findByCode("JUnitProductGroup"), "TX_PEN_ATRASO_PAG",
                    ...
                    PaymentPlanTestsUtilities.ls("Unidade"), true, false, 0, 
                    VatType.findByCode("INT"),
            ...
            FinantialInstitution.findAll().flatMap(fi -> FinantialEntity.find(fi)).forEach(fe -> {
                FixedTariff.create(Product.findUniqueByCode("TX_PEN_ATRASO_PAG").get(), null, fe, 
                        ...
                        DueDateCalculationType.DAYS_AFTER_CREATION, 
                        null, 0, false);
            });
        }
        
        FinantialInstitution.findAll().flatMap(fi -> FinantialEntity.find(fi)).forEach(fe -> {
            PaymentPenaltyTaxSettings settings = PaymentPenaltyTaxSettings.create(fe, Product.findUniqueByCode("TX_PEN_ATRASO_PAG").get());
            ...
            settings.edit(true, settings.getPenaltyProduct(), 
                    PaymentPlanTestsUtilities.ls("[Actos Administrativos] Pagamento em atraso - ${debitEntryDescription}"),
                    false, false);
        });
        
        GlobalInterestRate.findAll().forEach(i -> i.delete());
        GlobalInterestRate.create(new LocalDate(1950, 1, 1), 
                PaymentPlanTestsUtilities.ls("Juro oficial para o ano 2021"), new BigDecimal("4.705"),
                true, false);
    }
    ...
    public void interestsAfter_penaltyBefore_I2_D1_1000() {
        ...
        ISettlementInvoiceEntryBean penaltyTaxEntryBean1 = PaymentPlanTestsUtilities.createPenaltyTaxEntryBean(debitEntry, new BigDecimal("20.00"));
    ...
    public void interestsAfter_penaltyBefore_I5_D1_1000_D2_1_27() {
        ...
        ISettlementInvoiceEntryBean penaltyTaxEntryBean1 = PaymentPlanTestsUtilities.createPenaltyTaxEntryBean(debitEntry1, new BigDecimal("20.00"));
